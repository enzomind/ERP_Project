...
3. MYSQL로 관계형 DB를 구성했고 지출결의서 작성 시, 회계전표 생성, 서로 다른 2개 테이블의 통합 리스트 구현을 위해 UNION등의 쿼리 사용<br>
4. 완성도에 있어 중요한 View단은 부트스트랩 테마를 이용하여 공수를 최대한 줄이고 자바스크립트, 서비스 구현 등의 로직 개발에 집중<br>

<h3>T.F ❣️</h3>
5명 (메뉴별 1인 개발 진행)<br/>

<h3>담당 개발</h3>
1. 공통 : 공통 레이아웃 정의, 기간 조회 공통 스크립트 개발 및 기획(IA 정의 & 화면 기획 & 정책 정의) <br>
2. 메인 : 대시보드 구현 및 휴가자 캘린더 구현<br>
3. 회계 전표 : fetch.then 을 통해 Restful 리스트 조회 구현, 기간 조회와 서브 리스트 조회 구현<br>
